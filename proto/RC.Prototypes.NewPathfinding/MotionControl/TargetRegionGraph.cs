...
{
    class TargetRegionGraph : IGraph<Cell>
    {
        /// <summary>
        /// Constructs a TargetRegionGraph for a low-level pathfinding.
        /// </summary>
        /// <param name="currentRegion">The current region on the high-level path.</param>
        /// <param name="targetCell">The target cell.</param>
        public TargetRegionGraph(Region currentRegion, Cell targetCell)
        {
        ...
        {
            int horizontalDistance = Math.Abs(node.Coords.X - this.targetCell.Coords.X);
            int verticalDistance = Math.Abs(node.Coords.Y - this.targetCell.Coords.Y);
            int difference = Math.Abs(horizontalDistance - verticalDistance);
            return Math.Min(horizontalDistance, verticalDistance) * Grid.DIAGONAL_UNIT_DISTANCE + difference * Grid.STRAIGHT_UNIT_DISTANCE;
        }
