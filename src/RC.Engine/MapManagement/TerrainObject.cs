...
namespace RC.Engine
{
    ...
    /// </summary>
    class TerrainObject : ITerrainObject, ICellDataChangeSetTarget
    {
        /// <summary>
        ...
        public TerrainObject(Map map, TerrainObjectType type)
        {
            ...
            this.navCells = new NavCell[this.type.QuadraticSize.X * Map.NAVCELL_PER_QUAD,
                                        this.type.QuadraticSize.Y * Map.NAVCELL_PER_QUAD];
        ...
        public void Attach()
        {
            ...
            /// Set the references to the appropriate quadratic tiles and navigation cells.
            for (int quadX = 0; quadX < this.type.QuadraticSize.X; quadX++)
            {
                for (int quadY = 0; quadY < this.type.QuadraticSize.Y; quadY++)
                {
                    ...
                    {
                        QuadTile currQuadTile = this.parentMap.GetQuadTileImpl(this.mapCoords + relQuadCoords);
                        this.quadTiles[quadX, quadY] = currQuadTile;
                        for (int navX = 0; navX < Map.NAVCELL_PER_QUAD; navX++)
                        {
                            for (int navY = 0; navY < Map.NAVCELL_PER_QUAD; navY++)
                            {
                                RCIntVector relNavCoords = relQuadCoords * Map.NAVCELL_PER_QUAD + new RCIntVector(navX, navY);
                                this.navCells[relNavCoords.X, relNavCoords.Y] = currQuadTile.GetNavCellImpl(new RCIntVector(navX, navY));
                            }
                        }
                    }
                }
            ...
            /// TODO: Apply the navigation cell data changesets!
        ...
        public INavCell GetNavCell(RCIntVector index)
        {
            ...
            if (this.status != TerrainObjectStatus.Attached) { throw new InvalidOperationException("Invalid TerrainObject state!"); }
            if (index == RCIntVector.Undefined) { throw new ArgumentNullException("index"); }
            if (index.X < 0 || index.Y < 0 || index.X >= this.type.QuadraticSize.X * Map.NAVCELL_PER_QUAD || index.Y >= this.type.QuadraticSize.Y * Map.NAVCELL_PER_QUAD) { throw new ArgumentOutOfRangeException("index"); }
        ...
        public RCIntVector NavSize
        {
            get
            {
                return new RCIntVector(this.type.QuadraticSize.X * Map.NAVCELL_PER_QUAD,
                                       this.type.QuadraticSize.Y * Map.NAVCELL_PER_QUAD);
            }
        }
        ...
        /// <summary>
        /// List of the navigation cells of this terrain object.
        /// </summary>
